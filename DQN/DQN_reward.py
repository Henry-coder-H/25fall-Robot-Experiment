"""DQN for FrozenLake 8x8 with custom reward schedule and 1000 test episodes."""
import random
import time
...
TEST_EPISODES = 1000  # keep at 1000 per requirement
...
IS_SLIPPERY = True
REWARD_SCHEDULE = (5, -10, -1.00)  # goal, hole, step cost

PLOT_LOSS = False  # set False to skip loss curve
PLOT_LOSS_SMOOTH_WINDOW = 200  # moving average window for smoothing
...
def to_one_hot(state: torch.Tensor, num_states: int):
...
def compute_loss(policy_net, target_net, batch, n_states):
    ...
    return nn.MSELoss()(q_values, target_q)
...
def train():
    # Use a positive max_episode_steps to avoid immediate truncation; -1 in Gymnasium
    # causes TimeLimit to truncate at step 0.
    base_env = gym.make(
        "FrozenLake-v1",
        map_name="8x8",
    ...
    env = FrozenLakeRewardWrapper(base_env, REWARD_SCHEDULE)
...
def evaluate(policy_net: nn.Module, n_states: int, n_actions: int):
    ...
    eval_env = FrozenLakeRewardWrapper(base_env, REWARD_SCHEDULE)
...
if __name__ == "__main__":
    ...
    if PLOT_LOSS and loss_history:
        ...
        plt.ylabel("Loss")
        plt.title("DQN 8×8 reward Loss(Stochastic)")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("reward_8×8_stochastic.png", dpi=200)
        plt.close()
        print("Saved loss curve to loss_curve_8x8.png")
