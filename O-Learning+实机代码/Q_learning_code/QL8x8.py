...
MAX_STEPS_PER_EPISODE = 1000    # 每回合最大步数（防止一直走不结束）

LEARNING_RATE = 0.8           # Q-learning 学习率 α
DISCOUNT_FACTOR = 0.95         # 折扣因子 γ

# ε-greedy 探索参数
EPSILON = 1.0                  # 初始探索率（完全探索）
EPSILON_DECAY = 0.9995         # 每回合衰减比例
EPSILON_MIN = 0.01            # 最低探索率（不至于完全不探索）
...
env = gym.make(
    ...
    reward_schedule=(5, -10, -0.01),  # (到达终点, 掉洞, 冰面普通格) - 你的自定义奖励
...
for episode in range(NUM_EPISODES):
    ...
    # 这里假设：终点奖励为正（>10），掉洞奖励为负（<-10）
    episode_success.append(1 if final_reward > -10 else 0)
    episode_hole.append(1 if final_reward < -10 else 0)
