"""
Double DQN (with custom reward shaping) for FrozenLake-v1 with avg steps output. (Python 3.8 compatible)
Target:
...
def compute_loss_double(policy_net: nn.Module, target_net: nn.Module, batch, n_states: int) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        best_next_actions = q_next_online.argmax(dim=1, keepdim=True)
